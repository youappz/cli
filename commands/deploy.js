...
module.exports = program => {
  ...
  output.blankLine();
  output(
    'Deploy new YouAppz website version to stage ' + chalk.bold(deployStage)
  );
  output.blankLine();
  ...
  return Promise.resolve()
    ...
    .then(async tarballFile => {
      ...
      const version = await api.post({url, authToken: program.authToken, body: postBody});
      program.versionId = version.id
      await uploadTarballToS3(program, deployStage, tarballFile);
      return version
    })
    .then(version => waitForDeployComplete(program, deployStage, version))
    .then(version => {
      output.blankLine();
      var message = 'Version ' + version.id + ' deployment complete';
      if (
    ...
    .catch(err => {
      if (err.code === 'invalidManifest') {
        output.blankLine();
        output(
          '     ' + chalk.red('The youappz.yml has the following errors:')
        );
        output.blankLine();
...
function warnIfStaticGeneratorConfig(
  ...
  return fs.exists(path.join(deployDirectory, configFile)).then(exists => {
    ...
    output(
      '    $ ' + chalk.green('appz deploy --directory ' + outputDirectory)
    );
    output.blankLine();
    output(
      wordwrap(4, 70)(
        'Or you can set the directory option in the deploy section of your youappz.yml file:'
      )
    );
...
function createTarball(deployDirectory, program) {
  ...
  const tarballFile = path.join(program.cwd, 'appz-deploy.tar.gz');
...
function uploadTarballToS3(program, deployStage, tarballFile) {
  const spinner = startSpinner(program, 'Uploading archive to YouAppz');
  log.debug(
    'Invoke API to get temporary S3 presiged urls'
  );
  ...
  return Promise.resolve()
    .then(() => {
      return api
        .post({
          url: urlJoin(
            program.apiUrl,
            `/customers/${program.website.customerId}/signed-url`
          ),
          body: {key: program.website.appId + '/' + program.versionId + '.tar.gz',
          metadata: {
                  stage: deployStage,
    ...
    .then(async result => {
      // Use temporary signedurls to upload
      const payload = createReadStream(tarballFile);
      return fetch(result.signedUrl, {
        method: "PUT",
        body: payload,
        headers: {
             "Content-Type":"application/tar+gzip",
             "Content-Length": statSync(tarballFile).size
        }
      });
    })
    .then(res => {
      if(res.status !== 200) {
        spinner.fail();
        throw Error.create('Error uploading : ' + res.statusText, {});
      }
      spinner.succeed();
    })
    .catch(err => {
      spinner.fail();
      throw Error.create('Error uploading : ' + err.message, {}, err);
    });
}
